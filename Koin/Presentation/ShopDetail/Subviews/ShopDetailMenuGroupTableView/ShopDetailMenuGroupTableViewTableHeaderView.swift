...
final class ShopDetailMenuGroupTableViewTableHeaderView: UIView {
    
    // MARK: Properties
    let didScrollImageCollectionViewPublisher = PassthroughSubject<IndexPath, Never>()
    ...
    func configure(orderShopSummary: OrderShopSummary) {
        infoView.configure(orderShopSummary: orderShopSummary)
        imagesCollectionView.configure(orderImage: orderShopSummary.images)
    }
    func configure(orderShopMenusGroups: OrderShopMenusGroups) {
        menuGroupNameCollectionView.configure(menuGroup: orderShopMenusGroups.menuGroups)
    }
}


extension ShopDetailMenuGroupTableViewTableHeaderView {

...
//            self?.outputSubject.send(.showShopReviewStatistics(response.statistics))
//        }.store(in: &subscriptions)
//    }
//    
...
//        } receiveValue: { [weak self] response in
//            guard let self = self else { return }
//            self.outputSubject.send(.showShopReviewList(response.reviews, self.shopId, self.shopName, self.fetchStandard.0, self.fetchStandard.1, response.currentPage, response.totalPage, disappear))
//            self.outputSubject.send(.showShopReviewStatistics(response.statistics))
//            self.updateReviewCount()
//        }.store(in: &subscriptions)
//    }

...
extension TimetableViewModel {
    ...
    private func deleteLectureById(lecture: LectureData) {
        ...
        deleteLecturByIdUseCase.execute(id: lecture.id).sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] _ in
                self?.lectureData.removeAll {
                    $0.classTime == lecture.classTime && $0.name == lecture.name && $0.professor == lecture.professor
                }
            }
        ).store(in: &subscriptions)
    }
    ...
    private func _deleteLecture(_ lecture: LectureData) {
        deleteLectureUseCase.execute(frameId: selectedFrameId ?? 0, lectureId: lecture.id).sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] _ in
                self?.lectureData.removeAll {
    ...
    private func fetchFrames() {
        fetchFramesUseCase.execute().sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] response in
...
extension TimetableViewModel {
    ...
    private func postCustomLecture(lectureName: String, classTime: [Int]) {
        ...
        postLectureUseCase.execute(request: request).sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] response in
                self?.lectureData = response
            }
    ...
    private func postLecture(lecture: LectureData) {
        ...
        postLectureUseCase.execute(request: lectureRequest).sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] response in
                self?.lectureData = response
            }
    ...
    private func fetchLecture(frameId: Int) {
        fetchLectureUseCase.execute(frameId: frameId).sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] response in
    ...
    private func fetchFrame(semester: String) {
        fetchFrameUseCase.execute(semester: semester).sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] response  in
    ...
    private func fetchLectureList(semester: String) {
        fetchLectureListUseCase.execute(semester: semester).sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] response in
...
extension TimetableViewModel {
    ...
    private func rollbackFrame(id: Int) {
        rollbackFrameUseCase.execute(id: id).sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] response in
                self?.fetchFrames()
            }
    ...
    private func deleteFrame(frame: FrameDto) {
        deleteFrameUseCase.execute(id: frame.id).sink(
...
//                self?.nextOutputSubject.send(.showToastWithId(frame.timetableName, frame.id))
    ...
    private func modifyFrame(frame: FrameDto) {
        modifyFrameUseCase.execute(frame: frame).sink(
            receiveCompletion: { [weak self] completion in
                if case let .failure(error) = completion {
                    self?.nextOutputSubject.send(.showToast(error.message))
                }
    ...
    private func createFrame(semester: String) {
        createFrameUseCase.execute(semester: semester).sink(
            receiveCompletion: { [weak self] completion in
                if case let .failure(error) = completion {
                    self?.nextOutputSubject.send(.showToast(error.message))
                }
    ...
    private func deleteSemester(semester: String) {
        deleteSemesterUseCase.execute(semester: semester).sink(
            receiveCompletion: { _ in },
            receiveValue: { [weak self] _ in
                self?.fetchFrames()

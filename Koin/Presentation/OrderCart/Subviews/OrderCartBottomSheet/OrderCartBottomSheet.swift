...
final class OrderCartBottomSheet: UIView {
    ...
    func configure(shopMinimumOrderAmount: Int, itemsAmount: Int, finalPaymentAmount: Int, itemsCount: Int, isFromDelivery: Bool) {
        ...
        switch shopMinimumOrderAmount <= itemsAmount {
        case true:
            isDeliveryAvailableLabel.text = "배달 가능"
            configureButton(count: itemsCount, isEnabled: true)
        case false:
            let difference = formatter.string(from: NSNumber(value: shopMinimumOrderAmount - itemsAmount)) ?? "-"

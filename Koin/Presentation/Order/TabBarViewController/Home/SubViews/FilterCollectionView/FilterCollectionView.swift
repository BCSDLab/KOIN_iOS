...
final class FilterCollectionView: UICollectionView, UICollectionViewDataSource {
    ...
    func updateButtonState(_ standard: FetchOrderShopListRequest) {
        for (index, filter) in itemData.enumerated() {
            itemData[index].isSelected = standard.filter.contains(filter.type)
        }
        self.reloadData()
    }

//
//  ShopSortOptionSheetViewController.swift
//  koin
//
...
final class ShopSortOptionSheetViewController: UIViewController {
    ...
    private func makeOptionButtons() {
        ShopSortType.allCases.forEach { sort in
    ...
    private func refreshButtons() {
        for case let button as UIButton in optionStackView.arrangedSubviews {
            guard let sort = ShopSortType.allCases
    ...
    @objc private func optionTapped(_ sender: UIButton) {
        guard let sort = ShopSortType.allCases.first(where: { $0.hashValue == sender.tag }) else { return }
        current = sort
        refreshButtons()
...
private extension ShopSortOptionSheetViewController {
    ...
    private func setUpConstraints() {
        ...
        seperateView2.snp.makeConstraints {
            $0.bottom.equalTo(view.safeAreaLayoutGuide) // safeAreaLayoutGuide로 변경
            $0.leading.trailing.equalToSuperview()
            $0.height.equalTo(0.5)
        }

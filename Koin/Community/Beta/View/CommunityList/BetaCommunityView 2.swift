...
struct BetaCommunityView<T: CommonArticle, C: CommonComment>: View{
    ...
    var body: some View {
        return LoadingView(isShowing: self.$viewModel.progress) {
            List {
                ForEach(self.viewModel.dataSource) { r in
                    CommunityRow<T,C>(viewModel: r).environmentObject(self.user).environmentObject(self.viewModel)
                        .onAppear{
        ...
        .navigationBarItems(leading: Button(action: self.tabData.go_home) {
            ...
            }, trailing:  NavigationLink(destination: T.self == Article.self ? AnyView(RichEditor(is_edit: false, board_id: viewModel.boardId, token: self.user.get_token()).onDisappear{
